...
class PurchaseBillApp(tk.Tk):
    ...
    def create_custom_frame(self, width_ratio, height_ratio):
        ...
        tk.Label(custom_frame, text="self.Invoice_date.:", fg='Blue', bg='White',
    ...
    def submit_pressed(self, event):
        ...
        # print ("sl no =",len(self.scrollable_frame.grid_slaves(row=1, column=0)[0].get()))
        # print ("Desc = ",len(self.scrollable_frame.grid_slaves(row=1, column=2)[0].get()))
        ...
        for row in range(1, self.rows + 1):
            ...
            item = {
                ...
                "gst": self.scrollable_frame.grid_slaves(row=row, column=6)[0].get(),
                "dis": self.scrollable_frame.grid_slaves(row=row, column=7)[0].get(),
...
def backend(message):
    ...
    print("Backend thread is working...")
    ...
    try:
        ...
        destination = r"C:\Users\AMARJEET\Desktop\python_bill\AUDIT_Test\Audit" + invoice_no + ".xlsx"
